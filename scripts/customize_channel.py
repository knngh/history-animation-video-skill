...
def domestic_platform_rows() -> str:
    rows = [
        ("抖音", "最快测试爆款钩子", "35-75 秒", "前三秒强冲突，封面字狠一点"),
        ("快手", "信任和泛人群扩散", "45-90 秒", "讲清楚人物处境，多一点人情味"),
        ("微信视频号", "微信社交转发", "45-120 秒", "标题更稳，适合群分享和公众号联动"),
        ("小红书", "视觉审美和收藏", "30-75 秒", "封面干净，适合女性人物、服饰、冷知识"),
        ("B站", "系列沉淀和社区", "90-240 秒", "加资料、地图、弹幕友好解释"),
        ("西瓜视频", "中视频解释版", "1-5 分钟", "短视频验证后扩写上下文"),
        ("知乎", "观点和搜索权威", "1-5 分钟", "问答式标题，事实和推断分开"),
        ("微博", "热点讨论", "30-90 秒", "一句争议观点 + 话题标签"),
        ("百家号", "百度搜索流量", "1-5 分钟", "关键词标题，适合常青人物故事"),
    ]
    return "\n".join(f"- {name}：{role}；建议 {duration}；改写重点：{note}。" for name, role, duration, note in rows)
...
def make_plan(args: argparse.Namespace) -> str:
    ...
    return dedent(
        ...
        {domestic_platform_rows()}

        国内优先级建议：先发 抖音 + 小红书 + B站；如果内容偏大众和情绪传播，加快手和视频号；表现好的选题再扩写到西瓜、知乎、百家号；微博用于热点借势和讨论扩散。
